...
const [wire1, wire2] = rawInput
    .trim()
    .split('\n')
    .map(wire => wire
        .split(',')
        .map(op => [op[0], parseInt(op.slice(1))]));
const [wire1Points, wire1DistMapping] = createPointSet(wire1);
const [wire2Points, wire2DistMapping] = createPointSet(wire2);
const intersections = [...wire1Points].filter(p => wire2Points.has(p));

function createPointSet(wireArray) {
    ...
    wireArray.forEach(([dir, dist]) => {
        ...
        for (i = 0; i < dist; i++) {
            ...
            pointSet.add(p);
            distMapping[p] = distMapping[p] || d;
        }
    })

    return [pointSet, distMapping];
}

function part1() {
    const intersectionDists = intersections.map(p => {
        const [x, y] = p.split(',');
        return Math.abs(parseInt(x)) + Math.abs(parseInt(y));
    });
    return Math.min(...intersectionDists);
}

function part2() {
    const intersectionSteps = intersections.map(p => wire1DistMapping[p] + wire2DistMapping[p]);
    return Math.min(...intersectionSteps);
}
